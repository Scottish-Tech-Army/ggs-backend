...
const DB_GET_UNIT_RESPONSE = { Item: { "unitId": { "S": "Test Team" }, "locations": { "L": [] } } }



const LOCATIONS = [
    { locationId: "Old College", area: "Edinburgh", description: "Description 1", latitude: 55.9472097, longitude: -3.1892527, name: "Old College", photos: [{ isMain: true, url: "https://URL1" }] },
    { locationId: "Arthur's Seat", area: "Edinburgh", description: "Description 2", latitude: 55.9474659, longitude: -3.1777573, name: "Arthur's Seat", photos: [{ isMain: true, url: "https://URL2" }] },
    { locationId: "Brig o'Balgownie", area: "Aberdeen", description: "Description 3", latitude: 57.1772, longitude: -2.0985, name: "Brig o'Balgownie", photos: [] }]
...
const DYNAMO_LOCATIONS = [

    {
        ...
        "photos": {
            "L": [
                { "M": { "isMain": { "BOOL": true }, "url": { "S": "https://URL1" } } }
            ]
        }
    },
...
describe('api call POST /unit/login', () => {
    ...
    it('successful response (whitespace insensitive)', async () => {
        ...
        expect(result.statusCode).toEqual(200);
        expect(result.body).toEqual(JSON.stringify({ unitName: "\t \tTest Team 2" }));
    });
...
describe('api call GET /locations', () => {
    ...
    it('successful response', async () => {
        ...
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce(DB_GET_UNIT_RESPONSE);
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce(DB_LOCATIONS_RESPONSE);
    ...
    it('successful response with unit missing locations array', async () => {
        ...
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce({ Item: { "unitId": { "S": "Test Team" } } });
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce(DB_LOCATIONS_RESPONSE);
    ...
    it('successful response with collected locations', async () => {
        ...
        expect(result.statusCode).toEqual(200);
        expect(result.body).toEqual(JSON.stringify([
            { ...LOCATIONS[0], collected: true }, LOCATIONS[1], { ...LOCATIONS[2], collected: true }
        ]));
    });
...
describe('api call GET /unit/collect', () => {


    it('successful response', async () => {

        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce(DB_GET_UNIT_RESPONSE);
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce({ Item: DYNAMO_LOCATIONS[1] });
        ...
        expect(dynamodbClient.send).toHaveBeenNthCalledWith(3, expect.objectContaining({
            input: {
                Item: {
                    unitId: { S: "Test Team" },
                    locations: {
    ...
    it('successful response with already collected location', async () => {
        ...
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce({ Item: DYNAMO_LOCATIONS[0] });
    ...
    it('failure response with unknown collected location', async () => {
        ...
        expect(result.statusCode).toEqual(404);
        expect(result.body).toEqual(JSON.stringify({
            message: 'location not found: Unknown Location', request: event
        }));
    ...
    it('failure response on null body', async () => {
        ...
        expect(result.body).toEqual(JSON.stringify({
            message: "Invalid request body",
            request: {
                body: null, headers: { ggsunit: "Test Team" },
                resource: "/unit/collect",
            }
        }));
    });
    ...
    it('failure response on empty body', async () => {
        ...
        expect(result.body).toEqual(JSON.stringify({
            message: "Invalid request body",
            request: {
                body: "", headers: { ggsunit: "Test Team" },
                resource: "/unit/collect",
            }
        }));
    });
    ...
    it('failure response on invalid body', async () => {
        ...
        expect(result.body).toEqual(JSON.stringify({
            message: "Invalid request body",
            request: {
                body: "broken json", headers: { ggsunit: "Test Team" },
                resource: "/unit/collect",
            }
        }));
    });
...
describe('api call GET /unit/leaderboard', () => {
    ...
    it('successful response', async () => {
        ...
        expect(result.statusCode).toEqual(200);
        expect(result.body).toEqual(JSON.stringify(
            [{area:"Edinburgh",percentageCollected:0},{area:"Aberdeen",percentageCollected:0}]
        ));
    ...
    it('successful response with collected locations', async () => {
        ...
        expect(result.statusCode).toEqual(200);
        expect(result.body).toEqual(JSON.stringify(
            [{area:"Edinburgh",percentageCollected:50},{area:"Aberdeen",percentageCollected:100}]
        ));
    });
        ...
        (dynamodbClient.send as jest.Mock).mockResolvedValueOnce(
            {
                Item: {
                    unitId: { S: "Test Team" },
                    locations: {
                        L: [{ S: "Unknown Location" }, { S: "Old College" }, { S: "Brig o'Balgownie" },]
                    }
                }
            }
        );
