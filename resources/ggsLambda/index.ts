...
const getLocations = (): Promise<GGSLocation[]> => dynamodbClient.send(new ScanCommand({
  TableName: locationsTableName,
  ReturnConsumedCapacity: "TOTAL",
})).then((result) => result.Items!.map((item) => unmarshall(item) as GGSLocation))

const getUnitIds = () => dynamodbClient.send(new ScanCommand({
  TableName: unitsTableName,
  ReturnConsumedCapacity: "TOTAL",
  ProjectionExpression: "unitId"
})).then((result) => result.Items!.map((item) => unmarshall(item).unitId))

const getUnit = (unitId: string): Promise<GGSUnit | undefined> => dynamodbClient.send(new GetItemCommand({
  TableName: unitsTableName,
  Key: marshall({ unitId }),
})).then((result) => result.Item ? unmarshall(result.Item) as GGSUnit : undefined);

const getLocation = (locationId: string): Promise<GGSLocation | undefined> => dynamodbClient.send(new GetItemCommand({
  TableName: locationsTableName,
  Key: marshall({ locationId }),
})).then((result) => result.Item ? unmarshall(result.Item) as GGSLocation : undefined);

const updateUnit = (unit: Record<string, any>) => dynamodbClient.send(new PutItemCommand({
  TableName: unitsTableName,
  Item: marshall(unit),
}));
...
export const handler = async (event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
  ...
  if (event.resource === "/unit/login") {
    ...
    const unitName = unitIds.find((unitId) => unitId.trim().toLowerCase() === inputUnitName)
    if (!unitName) {
  ...
  if (event.resource === "/unit/collect") {
    ...
    if (unit.locations && unit.locations.includes(locationId)) {
      console.warn(`Unit ${unitName} has already collected location ${locationId}`);
  ...
  if (event.resource === "/unit/leaderboard") {
    ...
    locations.forEach(location => {
      const count = areaTotals[location.area] || 0;
      areaTotals[location.area] = count + 1;

      locationAreas[location.locationId] = location.area;
    })
    ...
    const collectedPercentages = areas.map(area => ({
      area,
      percentageCollected: Math.round(100 * (collectedTotals[area] || 0) / areaTotals[area])
    }))
